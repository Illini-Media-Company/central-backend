...
from db.content_doc import add_story, get_all_stories, get_story_by_id, update_story

# Initialize logger for this module
logger = logging.getLogger(__name__)

content_doc_routes = Blueprint("content-doc", __name__, url_prefix="/content-doc")


@content_doc_routes.route("/dashboard", methods=["GET"])
@login_required
def dashboard():
    """
    ...
    return render_template(
        "content_doc.html", currentUserName=user_name, stories=active_stories
    )


@content_doc_routes.route("/submit", methods=["POST"])
@login_required
def submit_story():
    """
    ...
    """
    logger.info(
        "[Submit Story] User %s is attempting to create a new story.",
        current_user.email,
    )
    ...
    if not title or not writer or not writer_email or not department:
        logger.warning(
            "[Submit Story] Failed validation: Missing required fields. Data received: %s",
            data,
        )
        return "Missing required story information.", 400

    logger.debug(
        "[Submit Story] Validation passed. Adding story '%s' to database.", title
    )
    ...
    logger.debug(
        "[Submit Story] Attempting to send Slack assignment notification to %s.",
        writer_email,
    )
    ...
    if not res.get("ok"):
        logger.error(
            "[Submit Story] Story created, but failed to notify writer %s via Slack.",
            writer_email,
        )
        return "Story created, but failed to notify writer.", 400

    logger.info(
        "[Submit Story] Successfully created story and notified writer %s.",
        writer_email,
    )
    return "Story successfully created and writer notified.", 200


@content_doc_routes.route("/<uid>/update", methods=["POST"])
@login_required
def update_story_status(uid):
    ...
    """
    logger.info(
        "[Update Story] User %s is attempting to update story ID: %s.",
        current_user.email,
        uid,
    )
    data = request.get_json()
    ...
    if writer_status is not None:
        update_data["writer_status"] = writer_status
    if copy_status is not None:
        update_data["copy_status"] = copy_status
    if google_doc_link is not None:
        update_data["google_doc_link"] = google_doc_link
    if snow_link is not None:
        update_data["snow_link"] = snow_link

    if not update_data:
        logger.warning(
            "[Update Story] Failed validation for story %s: No valid data provided to update.",
            uid,
        )
        return "No data provided to update.", 400

    logger.debug(
        "[Update Story] Attempting database update for story %s with data: %s",
        uid,
        update_data,
    )
    ...
    try:
        ...
        logger.info(
            "[Update Story] Successfully updated story %s in the database.", uid
        )
    ...
    if current_copy_status == "2nd edited" and current_snow_link:
        logger.info(
            "[Update Story] Workflow condition met: Story %s is '2nd edited' with a snow link. Notifying copy chief.",
            uid,
        )

        res = send_copy_chief_notification(
            updated_story.get("title"), current_snow_link
        )
        if not res.get("ok"):
            logger.error(
                "[Update Story] Story %s updated, but failed to notify copy chief via Slack.",
                uid,
            )
            return "Story updated, but failed to notify copy chief.", 400
        logger.debug(
            "[Update Story] Successfully notified copy chief for story %s.", uid
        )
