...
def send_claimer_confirmation(
    *,
    ...
    msg_text = (
        ...
        "When the graphics are ready and uploaded, reply to this message with the *Google Drive folder* link, or use the IMC Console."
    )
...
def ensure_claim_button(
    ...
    We support action_id: 'graphic_claim'.
        ...
        "elements": [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": "Claim this graphic",
                    "emoji": True,
                },
                "action_id": "graphic_claim",  # Updated for graphics
...
def post_graphic_blocks(
    *,
    req: dict,
    blocks: List[Dict[str, Any]],
    request_id: str,
    fallback_text: str = "New graphic request",
) -> Dict[str, Any]:
    """
    Post a graphic request to Slack, ensure a Claim button exists,
    and persist Slack identifiers (channel, ts) onto the request
    so we can update the message later. Routes dynamically based on request type.
    """
    request_type = str(req.get("requestType", "")).lower()
    ...
    try:
        ...
        print(f"[graphic_request] post_graphic_blocks failed: {e}")
...
def update_message_blocks(
    ...
    try:
        ...
        print(f"[graphic_request] chat_update failed: {e}")
        return {"ok": False, "error": str(e)}
...
def _handle_claim_generic(ack, body, logger):
    ...
    try:
        ...
        if not user_id:
            logger.error("[graphic_claim] Missing user_id in action payload")
            return
        ...
        if not request_id:
            logger.error("[graphic_claim] Missing request_id in button value")
            try:
        ...
        try:
            ...
            logger.error(f"[graphic_claim] users_info failed for {user_id}: {e}")
        ...
        try:
            app.client.chat_postEphemeral(
                ...
                channel=channel_id,
                user=user_id,
                text="✅ You successfully claimed this graphic request!",
            )
        except Exception:
            pass

    except Exception as e:
        logger.error(f"[graphic_slack] claim handler error: {e}")


@app.action("graphic_claim")
def _graphic_claim(ack, body, logger):
    _handle_claim_generic(ack, body, logger)


def build_blocks_from_request(req: dict) -> tuple[list, str]:
    """
    Build Slack Block Kit for a graphic request with your exact style and rules.
    ...
    blocks: list = [
        ...
        },
        {
            ...
            "text": {
                "type": "mrkdwn",
                "text": f"New graphic request from {submitter_mention}{di_dept_suffix}:",
            },
        },
        {"type": "divider"},
    ]
    ...
    fallback_text = (
        f"New graphic request from {submitter_name} for {dest}{di_dept_suffix}."
    )
                        ...
                        "elements": [
                            {
                                ...
                                "elements": [
                                    {
                                        "type": "text",
                                        "text": f"Graphic due by {due_text}",
                                        "style": {"bold": True},
                                    }
                                ],
                            }
...
def _on_dm_message(body, logger, event):
    ...
    Handles replies in a thread by the designer when they complete
    a graphic request. First checks that the message was in a direct message
    channel, then checks that it was a reply in a thread. Checks that the
    parent message in the thread is relevant to any graphic request. Validates
    that the user provided input is a valid Google Drive URL, then replies
    to confirm receipt of the message. Also completes the graphic request.
    """
    ...
    try:
        ...
        print("[slack_dm] Identified DM for current graphic request.")
...
def claim_request(uid: int, name: str, email: str):
    """
    Claim a graphic request. Called by both the API and when a user clicks the claim
    ...
    :type uid: int
    :param name: The name of the designer that claimed the request
    :type name: string
    :param email: The email address of the designer that claimed the request
    :returns: Message and HTTP status code
    :rtype: tuple
    """

    # DB: mark as claimed
    updated = claim_graphic_request(uid=int(uid), photogName=name, photogEmail=email)
    if not updated:
    ...
    try:
        ...
        if submitter:
            ...
            dm_user_by_email(
                email=submitter,
                text=f"🎨 Your graphic request \"*{label}*\" has been *claimed* by <@{updated.get('photogSlackId')}>.",
            )
    except Exception as e:
...
def complete_request(uid: int, driveURL: str):
    """
    Complete a graphic request. Called by both the API and when a user replies to a
    ...
    new completed status as well as the designer and completion timestamp.
    ...
    :type uid: int
    :param driveURL: The URL of the folder containing the graphics
    :type driveURL: string
    :returns: Message and HTTP status code
    :rtype: tuple
    """
    # DB: mark as completed
    updated = complete_graphic_request(uid=int(uid), driveURL=driveURL)
    if not updated:
        return {"error": "not found"}, 400
    ...
    try:
        ...
        if submitter:
            label = _label_for_request(updated)
            message_text = f'✅ Your graphic request "*{label}*" has been completed!'

            if updated.get("destination") not in ("The Daily Illini", "Illio Yearbook"):
                message_text += " Here's the link to your graphics: "
    ...
    if claim_ch and claim_ts:
        try:
            res = app.client.chat_postMessage(
                ...
                text=f"✅ I got the link! This graphic request is now complete.\n{updated.get('driveURL')}",
                thread_ts=claim_ts,
            )
            return {"ok": True, "channel": res["channel"], "ts": res["ts"]}, 200
        except Exception as e:
            print(f"[graphic_request] DM by id failed: {e}")
...
def delete_request(uid: int):
    """
    Delete a graphic request. Called only by API. Deleted the request via the
    ...
    # DB: delete request
    deleted = delete_graphic_request(uid=int(uid))
    if not deleted:
            ...
            blocks: list = [
                {
                    ...
                    "text": {
                        "type": "mrkdwn",
                        "text": f"🗑️ This graphic request \"*{label}*\" was deleted on {ap_datetime(datetime.now(ZoneInfo('America/Chicago')))}.",
                    },
                }
            ]
            fallback_text = f'🗑️ This graphic request "*{label}*" was deleted.'

            update_message_blocks(ch, ts, blocks, text=fallback_text)

            print("[delete_request] Updated message to graphic channel.")
    ...
    try:
        ...
        if submitter:
            message_text = f'🗑️ Your graphic request "*{label}*" was deleted. If you believe this was an error, please contact a graphic editor.'
    ...
    if claimer:
        try:
            # Delete the message originally telling them that they claimed a graphic
        ...
        try:
            message_text = f'🗑️ A graphic request you claimed "*{label}*" was deleted.'
